$(document).ready(function () {
  ...
  $(".link").click(function () {
    ...
    if ($(this).attr('id') == 'sortdtl') {
      $(".uppertitles").hide(0);
      $(".dtltitle").show(400);
    }
    if ($(this).attr('id') == 'sortname') {
      $(".uppertitles").hide(0);
      $(".yeartitle").show(400);
    }
  ...
  function ScrollAndPlayByLetter(letter) {
    ...
    $("#letterdata").fadeTo('slow', 0.7, function () {
      $("#letterdata").fadeTo('fast', 1, function () {});
    });
  ...
  function ChangePlayerMusic(val) {
    ...
    // {
    if (Currval != val) {
      $("#soundframe").attr("src", "https://www.youtube.com/embed/" + val + "?wmode=transparent&rel=0&autoplay=1");
      Currval = val;
      currdiv.removeClass('isselected');
    }
    // } else {
  ...
  $(document).keydown(function (e) {
    ...
    if (key == 38) {
      $('html, body').animate({
        scrollTop: ($(window).scrollTop() - ScrollHit) + 'px'
      }, 300);
    }
    if (key == 40) {
      $('html, body').animate({
  ...
  if (typeof QueryString.id !== 'undefined') {
    setTimeout(function () {
      ScrollAndPlayById(QueryString.id);
    }, 2400);
  ...
  setTimeout(function () {
    $('#itemlist').mixItUp('sort', 'year:desc');
  }, 500);
